...
import { closeModalEditEmployee } from '../../store/modalEditEmployeeSlice';
...
function ModalEditEmployee({ employee }: ModalEditEmployeeProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        dispatch(closeModalEditEmployee());
        setFormData({ photo: '', fullName: '', position: '', admission: '', phone: '' });
        dispatch(clearError());
      }
    };

    if (isModalOpenEditEmployee) {
  ...
  }, [isModalOpenEditEmployee, dispatch, pathName]);
